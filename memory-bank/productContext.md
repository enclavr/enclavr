# Product Context - Enclavr

## Why This Project Exists
Existing voice chat platforms either require cloud hosting (with associated costs and data privacy concerns) or lack self-hosted options. Organizations and individuals need a private, controllable voice communication solution.

## Problems It Solves
1. **Privacy Concerns**: Users want to control their own data without third-party servers
2. **Cost**: Enterprise voice solutions are expensive
3. **Control**: Organizations need full control over their communication infrastructure
4. **Self-Hosting**: Tech-savvy users prefer self-hosted solutions for flexibility

## User Experience Goals
- Simple setup process
- Intuitive room-based voice communication
- Low latency audio streaming
- Cross-platform compatibility
- Modern, responsive UI

## Target Users
- Small to medium organizations
- Gaming communities
- Development teams
- Privacy-conscious individuals
- Enterprises requiring private communication
